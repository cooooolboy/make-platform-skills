...
- Local preview is the only exception: Service may read makecli credentials when `MAKE_APP_LOCAL_PREVIEW=true`, attach the token only to Service-to-Make requests, and return preview-marked current-context/runtime-view responses. This path must not be enabled in production and must not expose tokens through UI, `/api/config`, logs, or build artifacts.
