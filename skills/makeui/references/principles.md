...
Preserve the host project's declared data flow. New generated Make Apps default to unified login through `@qfeius/make-app-auth`; token mode is only an explicit local/debug override. If project instructions, `apps/docs/api.md`, or existing code require `apps/ui -> apps/service -> Make Data API`, UI code must use the Service API contract and must not directly call Make APIs or hold Make credentials. Service-fronted unified-login Apps use `@qfeius/make-app-auth`: generated UI calls `auth.api('/app/**')`, the browser reaches same-origin `/api/make/app/**`, and App Service performs the Make backend orchestration. If the user is generating a direct gateway/unified-login Make App, authentication and `/api/make/**` access must be handled through `@qfeius/make-app-auth` and the separate `make-app-auth` skill: generated UI calls `auth.api('/data/**')` or `auth.api('/meta/**')`, then reaches make-gateway and Make Platform. UI code must not hold Make credentials, bypass `auth.api`, directly call meta/data service domains, call k8s-internal service names, or silently replace a Service-based contract with the gateway/auth-SDK flow. `apps/service` remains part of the required project structure.
