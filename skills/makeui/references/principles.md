...
Preserve the host project's declared data flow. Authentication mode and `/api/make/**` request behavior belong to `make-app-auth`; in the current Make App generation baseline, new generated Apps use unified login by default and token mode only as an explicit local/debug override. If project instructions, `apps/docs/api.md`, or existing code require `apps/ui -> apps/service -> Make Data API`, UI code must use the Service API contract and must not directly call Make APIs or hold Make credentials. If the host project is Service-fronted unified login, preserve that contract: UI calls the Service API through the project auth wrapper or `auth.api`, Service owns calls to make-gateway, and UI must not bypass Service by calling `/data/**` or `/meta/**` directly. If the project is a direct gateway/unified-login Make App, coordinate authentication and `/api/make/**` behavior through `make-app-auth`. UI code must not hold Make credentials, bypass `auth.api`, directly call meta/data service domains, call k8s-internal service names, or silently replace a Service-based contract with the gateway/auth-SDK flow. `apps/service` remains part of the required project structure.
